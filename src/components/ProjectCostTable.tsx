import { Box, Card, CardContent, Paper, SxProps, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Theme, makeStyles } from "@mui/material";
...
const ProjectCostTable = ({ project }: { project: Project }) => {
					...
					<TableBody>
						{summary.map((s) => (
							<TableRow key={s.memberId}>
								...
								<TableCell sx={tsx}>{numberWithSpaces(s.totalPrice)} €</TableCell>
							</TableRow>
						))}
                        <TableRow >
                            ...
                            <TableCell sx={fsx}>{numberWithSpaces(totals.totalPrice)} €</TableCell>
                        </TableRow>
...
const PhaseCell = ({ phaseId, project, summary, sx }: { phaseId: string; project: Project; summary: MemberSummary, sx?: SxProps<Theme> }) => {
    ...
    const s = sx || {}
	return <TableCell sx={s}>{sum ? numberWithSpaces(sum[2]) : 0} €</TableCell>;
};
